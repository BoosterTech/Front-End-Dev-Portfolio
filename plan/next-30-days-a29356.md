...
4. ✅ Move `src/features/portfolio/Projects/projects.js` data to `src/content/projects.js` using `jsconfig.json` `baseUrl`; image imports now use `images/...`.

## Week 2 — Structure

1. ✅ Add a `jsconfig.json` with `baseUrl: "src"` (custom `@/` paths require eject/CRACO, so use direct `src`-relative imports). Then move `projects.js` to `content/projects` and image imports to `images/...`.
2. Replace deep relative imports (`../../../...`) with `src`-relative imports (`content/...`, `images/...`) where touched.
...
- `npm run build` and `CI=true npm test -- --watchAll=false` pass without warnings.
- No unused dependencies in `package.json`.
- No dead code in `src/common/`.
- All data files live under `src/content/` (skillsets and projects now moved).
- CI runs on every PR.
