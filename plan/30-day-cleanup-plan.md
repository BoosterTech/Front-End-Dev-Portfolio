...
1. [ ] Move `StarField` and `Main` out of `src/GlobalStyles.js` into `src/common/`.
...
2. [ ] Split `translations` out of `src/themes.js`.
...
3. [ ] Make `StarField` deterministic.
...
2. [ ] Tighten `bundle-size.js` budget.
...
3. [ ] Review and document `framer-motion` usage.
...
1. [ ] Create a shared `Card` / `Panel` primitive in `src/common/Card/`.
...
2. [ ] Split `src/content/skillsets.js` by language.
...
3. [ ] Split `src/content/projects.js` by language if it grows, or keep as a single module under the documented size-check exclusion.
...
1. [ ] Add a coverage report or threshold to `npm test`.
...
2. [ ] Add Lighthouse CI or a Core Web Vitals check.
...
3. [ ] Update `plan/architecture-playbook.md`.
