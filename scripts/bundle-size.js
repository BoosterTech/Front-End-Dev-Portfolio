...
const MAX_SIZE = Number(process.env.BUNDLE_SIZE_LIMIT) || 600 * 1024;
...
function main() {
  ...
  for (const file of files) {
    ...
    if (size > MAX_SIZE) {
      console.error(
        `Bundle too large: ${relative} (${sizeKb} KB) exceeds ${MAX_SIZE / 1024} KB`
      );
      failed = true;
    } else {
      console.log(`OK: ${relative} (${sizeKb} KB)`);
    }
  }
